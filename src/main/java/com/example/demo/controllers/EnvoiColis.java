...
public class EnvoiColis {
    ...
    public ResponseEntity<List<Delivery>> getAllDeliveriesForSenders(@RequestHeader(name = "Authorization") String authorizationHeader) {
    ...
    public ResponseEntity<List<Package>> getPackagesFromDelivery(@PathVariable String deliveryId) {
        Delivery d = deliveryService.findByLabel_livraison(deliveryId);
        // Convert the Iterable to a List
        List<Package> packagesList = d.getPackages();
    ...
    public ResponseEntity<Object> createNewDelivery(@RequestBody DeliveryRequest deliveryRequest, @RequestHeader(name = "Authorization") String authorizationHeader) {
    	try {
            // Extract the objects from the deliveryRequest
            Bus bus = busService.loadBusById(deliveryRequest.getBus());
            System.out.println("Yo + : "+authorizationHeader);
            // Remove "Bearer " prefix to get the token
            String token = authorizationHeader.substring(7);
            ...
            // Retrieve the 'agence' attribute
            String agence = (String) claims.get("agence");
            System.out.println("Agency : "+agence);
            ...
            d.setVilleDepart(agence);
            d.setLabelLivraison(generatedString);
            d.setDepart(LocalDateTime.now());          //bus.setCity((String) session.getAttribute("ville"));
            d.setBus(bus);
